...
UP = '/root/.claude/uploads/d3882ddd-ac56-5272-922a-2a4856de9639/'
OUT = 'templates/'
...
def cls_flag(ws, row, C):
    ...
    for k in range(0, 12):
        ...
        if v: return labels[min(k, 11)].rstrip('2'), v
    return 'AI', 'A'
...
def parse_spl(path, sheet, hdr_row, data_row, meta_rows=True):
    ...
    for r in range(data_row, ws.max_row + 1):
        ...
        bit = None
        bm = num(g('bitmask'))
        if bm is not None and (regtype.lower() == 'boolean' or (addr_raw and '.' in addr_raw)):
            bit = int(bm)
        gain = num(g('gain'))
        vendor = g('vendor') or ''
...
save(mk_template('schneider-pm8000', 'SCHNEIDER', 'PM8000 Series', ['002.002.001'],
                 ...
                 'Schneider PM8000 Modbus Map (curated 27-point metering set)',
